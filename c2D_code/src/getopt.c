...
c2dOptions* get_options(int argc, char** argv) {
  ...
  while(1) {
    ...
    switch(argument) {
      ...
      default:  print_help(C2D_PACKAGE,1);
    }
  }
  ...
  if(options->cnf_filename==NULL) {
    fprintf(stderr,"%s: must specify a CNF file\n",C2D_PACKAGE);
    print_help(C2D_PACKAGE,1);
  }
  if(options->vtree_type!='p' && options->vtree_type!='i') {
    fprintf(stderr,"%s: option -t must be 'p' or 'i'\n",C2D_PACKAGE);
    print_help(C2D_PACKAGE,1);
  }
  if(options->vtree_method < 0 || options->vtree_method > 3) {
    fprintf(stderr,"%s: option -m must be 0, 1, 2, or 3\n",C2D_PACKAGE);
    print_help(C2D_PACKAGE,1);
  }
  if(options->vtree_count < 1) {
    fprintf(stderr,"%s: option -b must be greater than 0\n",C2D_PACKAGE);
    print_help(C2D_PACKAGE,1);
  }
  if(options->initial_ubfs < 1 || options->initial_ubfs > 49) {
    fprintf(stderr,"%s: option -u must be between 1 and 49 (inclusive)\n",C2D_PACKAGE);
    print_help(C2D_PACKAGE,1);
  }
  if(options->final_ubfs < 1 || options->final_ubfs > 49) {
    fprintf(stderr,"%s: option -f must be between 1 and 49 (inclusive)\n",C2D_PACKAGE);
    print_help(C2D_PACKAGE,1);
  }
  if(options->initial_ubfs > options->final_ubfs) {
    fprintf(stderr,"%s: option -u must be less than or equal to option -f\n",C2D_PACKAGE);
    print_help(C2D_PACKAGE,1);
  }
  if(options->cache_capacity < 1) {
    fprintf(stderr,"%s: option -s must be greater than 0\n",C2D_PACKAGE);
    print_help(C2D_PACKAGE,1);
  }
  return options;
}
...
void print_help(const char* PACKAGE, int exit_value) {
  printf("%s: CNF to Decision-DNNF Compiler\n", PACKAGE);
  printf("%s\n",c2d_version());
...
Copyright (c) Automated Reasoning Group, UCLA 2015\n\
